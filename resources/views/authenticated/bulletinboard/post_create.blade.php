    ...
    <div class="">
      ...
      <select class="w-100" form="postCreate" name="post_category_id">
        ...
        <optgroup label="{{ $main_category->main_category }}">
          @foreach($main_category->mainTosubCategory($main_category->id)->get() as $sub_category)
          <option name="sub_category_id" value="{{ $sub_category["id"] }}">{{ $sub_category["sub_category"] }}</option>
          @endforeach
        </optgroup>
        <!-- サブカテゴリー表示 -->
        @endforeach
  ...
  <div class="w-25 ml-auto mr-auto">
    <div class="category_area mt-5 p-5">
      <div class="">
        ...
        <!--/////////////サブカテゴリー追加フォーム/////////////-->
        <form action="{{ route('sub.category.create') }}" method="post" id="subCategoryRequest" name="subCategoryRequest">{{ csrf_field() }}
